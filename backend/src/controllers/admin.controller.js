...
exports.storeElectionInRemoteDb = async (req, res) => {
  try {
    ...
    // Create new candidate records for each candidate, regardless of whether they already exist
    const remoteCandidatesPromises = candidates.map(async (candidate) => {
      // Always create a new candidate record with a unique identifier
      const uniqueId = `${candidate._id.toString()}_${batchTimestamp}`;
